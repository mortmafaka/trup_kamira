...
from src.Predict import NN_Runner, XGBoost_Runner,LR_Runner  
...
def simple_voting(nn_preds, xgb_preds, lr_preds):
    final_preds = []
    for nn, xgb, lr in zip(nn_preds, xgb_preds, lr_preds):
        combined_pred = np.argmax(np.bincount([nn, xgb, lr]))
        final_preds.append(combined_pred)
    return final_preds

def print_final_predictions(predictions, games):
    for prediction, game in zip(predictions, games):
        home_team, away_team = game
        print(f"{home_team} vs {away_team}: Predicted outcome: {'Home Win' if prediction == 1 else 'Away Win'}")
...
def main():
    ...
    if args.lr:
        ...
        LR_Runner.lr_runner(data, todays_games_uo, frame_ml, games, home_team_odds, away_team_odds, args.kc)
        print("-------------------------------------------------------------------")
    ...
    if args.A:
        ...
        LR_Runner.lr_runner(data, todays_games_uo, frame_ml, games, home_team_odds, away_team_odds, args.kc)
        print("-------------------------------------------------------------------")
    # Запускаем модели и получаем прогнозы
        nn_predictions = NN_Runner.nn_runner(data, todays_games_uo, frame_ml, games, home_team_odds, away_team_odds, args.kc, return_predictions=True)
        xgb_predictions = XGBoost_Runner.xgb_runner(data, todays_games_uo, frame_ml, games, home_team_odds, away_team_odds, args.kc, return_predictions=True)
        lr_predictions = LR_Runner.lr_runner(data, todays_games_uo, frame_ml, games, home_team_odds, away_team_odds, args.kc, return_predictions=True)

    # Простое голосование
        final_predictions = simple_voting(nn_predictions, xgb_predictions, lr_predictions)
        print_final_predictions(final_predictions, games)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Model to Run')
    parser.add_argument('-xgb', action='store_true', help='Run with XGBoost Model')
    parser.add_argument('-nn', action='store_true', help='Run with Neural Network Model')
    parser.add_argument('-lr', action='store_true', help='Run with Logistic Regression Model')  # New argument for logistic regression
